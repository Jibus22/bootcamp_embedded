...
void i2c_write(unsigned char data) {
  ...
  /* Check value of TWI Status Register to see if data was sent and ACK
   * received. Mask prescaler bits. If status different from
   * MT_DATA_ACK go to ERROR. */
  if ((TWSR0 & 0xF8) != TM_MT_DATA_ACK) ERROR("WR data failed\n\r");
}
